...
def without_periods_average(signals, event, stimulate):
    ...
    for e in event:
        if 1 <= e[0] <= 12:
            start = e[1] - 1
            signal = signals[start: start + SCOPE]
            temp = [signal,
...
def read_train_data(subject, is_average=True, is_save=False):
    ...
    for i in range(len(CHAR)):
        sheet_name = 'char' + CHAR[i]
        print("处理字符%s的数据..." % CHAR[i])
        char_row_col = CHAR_ROW_COL[i]
    ...
    if is_save:
        ...
        file_name = 's%d_train_%s.pkl' % (subject, 'w' if is_average else 'wo')
...
if __name__ == '__main__':
    read_train_data(subject=1, is_average=True, is_save=False)
